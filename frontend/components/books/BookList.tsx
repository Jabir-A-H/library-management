...
function BookList({
  ...
  switch (sortBy) {
    ...
    case 'oldest':
      ...
      break;
    case 'favorites':
      filteredBooks.sort((a, b) => (b.isFavorite ? 1 : 0) - (a.isFavorite ? 1 : 0));
      break;
    default:
      filteredBooks.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
  }
  ...
  const handleToggleFavorite = (bookId: string | number) => {
    ...
    if (book) {
      updateBook.mutate({ id: bookId, ...book, isFavorite: !book.isFavorite });
    }
  };

  // Confirm and delete a book
  const handleDeleteBook = (bookId: string | number) => {
  ...
  const favoriteBooks = books.filter((book: Book) => book.isFavorite).length;
  const uniqueGenres = [...new Set(books.map((book: Book) => book.genre).filter(Boolean))].length;

  if (isLoading) {
    return <div className="py-16 text-center">Loading books...</div>;
  }
  if (isError) {
    return <div className="py-16 text-center text-red-500">Failed to load books.</div>;
  }
  if (books.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <BookOpen className="h-24 w-24 text-muted-foreground mb-4" />
        <h2 className="text-2xl font-semibold mb-2">No books in your library yet</h2>
        <p className="text-muted-foreground mb-6 max-w-md">
          Start building your personal library by adding your first book. You can include cover images,
          descriptions, tags, and more to organize your collection.
        </p>
        <Button onClick={onAddBook} size="lg" aria-label="Add your first book" title="Add your first book">
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="bg-card border border-border rounded-lg p-4 text-center">
          ...
          <div className="text-2xl font-bold text-foreground">{favoriteBooks}</div>
          <div className="text-sm text-muted-foreground">Favorites</div>
        </div>
        <div className="bg-card border border-border rounded-lg p-4 text-center">
          <TrendingUp className="h-8 w-8 mx-auto mb-2 text-green-500" />
          <div className="text-2xl font-bold text-foreground">{uniqueGenres}</div>
      ...
      <div className="flex justify-between items-center">
        ...
        <Button onClick={onAddBook} variant="default" aria-label="Add book" title="Add book">
