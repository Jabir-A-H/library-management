/**
 * Lending Record interface matching the backend SQLAlchemy model structure
 ...
 */
export type LendingRecordCreate = {
  book_id: number;
  borrower_id: number;
  due_date: string;
  notes?: string;
 ...
 */
export type LendingRecordUpdate = {
  due_date?: string;
  return_date?: string;
  status?: 'borrowed' | 'returned' | 'overdue' | 'lost';
  notes?: string;
 ...
 */
export interface LendingRecordSearchFilters {
  search?: string;
  status?: string;
