"""
User Favorite schemas for API requests and responses
"""
...
class UserFavoriteBooksResponse(BaseModel):
    user_id: int
    username: str
    favorite_books: List['BookSummaryResponse']
    total_favorites: int
...
class BookFavoritedByResponse(BaseModel):
    book_id: int
    book_title: str
    favorited_by: List['UserSummaryResponse']
    total_favorites: int
...
UserFavoriteResponse.model_rebuild()
